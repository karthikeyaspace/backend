...
shorten.post('/', asyncHandler(async (req, res) => {
    ...
    try{
        ...
        if(supares.data)
            return res.send({ message: "url already exists", payload: { shortUrl: supares.data.shortUrl } })
...
shorten.get('/:alias', asyncHandler(async (req, res) => {
    ...
    try{
        ...
        if(supares.data)
            return res.send({message: "redirecting", payload: {longUrl: supares.data.longUrl}})
        else
